...
def main() -> None:
    ...
    original_cap_list: list[str] = [name for name in initial_list
    ...
    score_this: dict[str, int] = {name: random.randint(0, 1000)
                                  for name in final_cap_list}
    print(f"Score dict: {score_this}")
    print(f"Score average: {sum(score_this.values()) / len(score_this):.2f}")
    highest_score: dict[str, int] = {
        name: score_this[name] for name in score_this if score_this[name]
        > sum(score_this.values()) / len(score_this)}
